...
def get_service_name(port):     # 포트를 기반으로 서비스 이름을 가져오는 함수
    try:
        return socket.getservbyport(port, "tcp")
    except OSError:
        return "unknown"
    
def get_basic_banner(targetIp, port, timeout):      # 기본 배너를 가져오는 함수 (SSL 제외)
    try:
...
def get_ssl_banner(targetIp, port, timeout):        # ssl 배너를 가져오는 함수
    try:
        with socket.create_connection((targetIp, port), timeout) as sock:
            if port == 443:
        ...
        return "No Banner"
def extract_server_header(response):        # 서버 헤더 추출
...
def scan_service_version(targetIp, port, timeout, maxTries):        # 대상Ip에 대한 포트 서비스 버전 스캔
